...
// All plugins are now in plugins/ (keeperhub/plugins/ was consolidated)
const KEEPERHUB_PLUGINS_DIR = PLUGINS_DIR;
const PROTOCOLS_DIR = join(process.cwd(), "protocols");
const OUTPUT_FILE = join(PLUGINS_DIR, "index.ts");
const KEEPERHUB_OUTPUT_FILE = join(PLUGINS_DIR, "index.ts");
...
/**
 * Generate the plugins/index.ts file (KeeperHub-specific plugins)
 */
...
async function main(): Promise<void> {
  ...
  if (keeperhub.all.length === 0) {
    console.log("\nNo KeeperHub plugins found in plugins/ directory");
  } else {
  ...
  generateIndexFile(base.enabled); // All plugins are now in plugins/
