...
describe.skipIf(!HAS_PARA)("Scenario B: Para signer accepts type-4 RLP", () => {
  it("attempts to sign a type-4 transaction via ParaEthersSigner", {
    timeout: 60_000,
  }, async () => {
    const { ethers } = await import("ethers");
    ...
    const { getOrganizationWallet } = await import("@/lib/para/wallet-helpers");
...
describe.skipIf(!HAS_ALL)(
  ...
  () => {
    ...
    it("C.a: extracts private key from Para and verifies address match", {
      timeout: 60_000,
    }, async () => {
      const { ethers } = await import("ethers");
      const { Environment, Para: ParaServer } = await import(
      ...
      const orgId = process.env.TEST_ORG_ID;
      if (!orgId) {
        console.log("SKIP: TEST_ORG_ID not set -- cannot test key extraction");
        return;
      }
    ...
    it("C.b: signs EIP-7702 authorization tuple with extracted key", {
      timeout: 30_000,
    }, async () => {
    ...
    it("C.c: submits sponsored EIP-7702 tx via Pimlico (ERC-4337 hybrid)", {
      timeout: 120_000,
    }, async () => {
      if (!(privateKeyHex && extractedAddress)) {
        console.log("SKIP: C.a did not produce a private key");
...
describe.skipIf(!HAS_ALL)(
  ...
  () => {
    it("D: creates and submits a sponsored UserOp via standard smart account", {
      timeout: 120_000,
    }, async () => {
      const { createPublicClient, http } = await import("viem");
      const { privateKeyToAccount } = await import("viem/accounts");
